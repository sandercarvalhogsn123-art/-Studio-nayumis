...
export function getAvailableSlots({open='08:00',close='19:30',duration=60,step=30,appointments=[],breaks=[]}){
  const out=[]; const o=minutes(open), c=minutes(close);
  for(let s=o;s<=c;s+=step){const e=s+duration; const busy=[...appointments,...breaks].some(x=>overlaps(s,e,minutes(x.start),minutes(x.end))); if(!busy) out.push(hhmm(s));}
  return out;
}
