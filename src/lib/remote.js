...
export async function loadRemote(base){
  ...
  try {
    ...
    const settings=bsd?{...base.settings,name:bsd.business_name||base.settings.name,subtitle:bsd.subtitle||'',phone:bsd.phone||'',instagram:bsd.instagram||'',address:bsd.address||'',primary:bsd.primary_color||base.settings.primary,background:bsd.hero_color||base.settings.background,hero:bsd.hero_color||base.settings.hero,logo:bsd.profile_image_url||bsd.logo_url||''}:base.settings;
